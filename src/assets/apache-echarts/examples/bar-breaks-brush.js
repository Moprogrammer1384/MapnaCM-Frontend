...
function initAxisBreakInteraction() {
    ...
    function updateAxisBreak(myChart, initXY, currPoint) {
        ...
        var gapPercentStr = (Math.abs(myChart.convertToPixel({ yAxisIndex: 0 }, newBreak.start)
            - myChart.convertToPixel({ yAxisIndex: 0 }, newBreak.end)) / getYAxisPixelSpan(myChart) * 100) + '%';
